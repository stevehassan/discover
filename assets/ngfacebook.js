  ...
  .provider('$facebook', function() {
    ...
    this.$get = ['$q', '$rootScope', '$window', function($q, $rootScope, $window) {
      ...
      $facebook.init = function() {
        ...
        $window.FB.init(
          angular.extend({appId:   $facebook.config('appId'),
						  status:  $facebook.config('status'),
						  cookie:  $facebook.config('cookie'),
						  xfbml:   $facebook.config('xfbml'),
						  version: $facebook.config('version')}, $facebook.config("customInit"))
        );
      ...
      $facebook.cachedApi = function() {
        if(typeof arguments[0] !== 'string')
          throw "$facebook.cacheApi works only with graph requests!";
